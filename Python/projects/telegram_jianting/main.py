...
# 配置One/New API
ONEAPI_KEY = "sk-wvSDlhtNqADvHM4NEs5HaI1P7jNGth2aV7VXSCsN8dUshIfV"  # 替换为实际的 API 令牌
ONEAPI_BASE_URL = "https://oneapi.oaiee.com//v1"  # API 的服务地址
                            ...
                            if log_file:
                                try:
                                    with open(log_file, 'a', encoding='utf-8') as f:
                                        f.write(
                                            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 对话{chat_id} 完全匹配关键词'{keyword}': {message_text}\n")
                        ...
                        if match_obj:
                            ...
                            if log_file:
                                try:
                                    with open(log_file, 'a', encoding='utf-8') as f:
                                        f.write(
                                            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 对话{chat_id} 正则匹配关键词'{keyword}': {message_text}\n")
                                    logger.info(f"已将匹配关键词 '{keyword}' 的消息写入文件: {log_file}")
                                except Exception as e:
                                    logger.error(f"写入文件 {log_file} 时发生错误：{e}")
                        ...
                        for row_i, row in enumerate(event.message.buttons):
                            for col_i, button in enumerate(row):
                                if b_keyword in button.text.lower():
                                    ...
                                    logger.info(
                                        f"已点击对话 {chat_id} 中包含按钮关键词 '{b_keyword}' 的按钮: {button.text}")
                                    return
...
def match_user(sender, user_set, user_option):
    ...
    logger.info(
        f"匹配用户：sender_id={sender_id}, sender_username={sender_username}, sender_full_name={sender_full_name}, user_set={user_set}")
    ...
    while True:
        ...
        try:
            if command == 'list':
                ...
                log_to_file = (await ainput(
                    "是否将匹配的消息文本写入指定的.txt文件？(yes/no): ")).strip().lower() == 'yes'
                if log_to_file:
                ...
                if match_type == 'regex':
                    regex_send = (await ainput(
                        "是否将正则匹配到的内容发送到指定对话？(yes/no): ")).strip().lower() == 'yes'
                    if regex_send:
                ...
                for keyword in keywords:
                    ...
                    if match_type == 'regex' and regex_send_target_id is not None:
                        ...
                        print(
                            f"正则匹配结果将发送到ID: {regex_send_target_id}, 随机延时: {regex_send_random_offset}秒, 发送后删除: {regex_send_delete}")
                ...
                if keyword_input in KEYWORD_CONFIG:
                    ...
                    if '7' in options:
                        log_to_file = (await ainput(
                            "是否将匹配的消息文本写入.txt文件？(yes/no): ")).strip().lower() == 'yes'
                        if log_to_file:
                    ...
                    if '8' in options and KEYWORD_CONFIG[keyword_input].get('match_type') == 'regex':
                        ...
                        if regex_send:
                            ...
                            random_offset_input = (await ainput("请输入随机延时（秒），默认为0: ")).strip()
                            regex_send_random_offset = int(random_offset_input) if random_offset_input else 0
                            regex_send_delete = (await ainput(
                                "是否在发送后删除该消息？(yes/no): ")).strip().lower() == 'yes'
                ...
                extensions_input = (
                    await ainput("请输入文件后缀名，多个后缀名用逗号分隔 (例如: .pdf,.docx): ")).strip().lower()
                ...
                if job:
                    ...
                    if sched:
                        ...
                        # 修改发送后删除消息
                        if '5' in options:
                            delete_after_sending = (await ainput(
                                "是否在发送后删除消息？(yes/no): ")).strip().lower() == 'yes'
                ...
                for sched in SCHEDULED_MESSAGES:
                    print(
                        f"Job ID: {sched['job_id']}, 目标ID: {sched['target_id']}, 消息: {sched['message']}, Cron: {sched['cron']}, 随机时间误差: {sched.get('random_offset', 0)} 秒, 发送后删除: {'是' if sched.get('delete_after_sending', False) else '否'}")
